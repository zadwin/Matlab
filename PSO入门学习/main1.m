...
% 用inline定义适应度函数以便将子函数文件与主程序文件放到一起，不用多次写这个函数
...
globalbest_x = personalbest_x(i,:);   % 这个是必定是全局最优点的位置，就是一个取的过程
k = 1; % 开始迭代计数
while k <= MaxNum   % 当迭代次数达到设定的最大值的时候，就不要再进行迭代了
   ...
   [globalbest_faval,i] = min(personalbest_faval); 
    ...
    for i = 1:particlesize
        v(i,:) = w*v(i,:) + c1*rand*(personalbest_x(i,:) - x(i,:)) + c2*rand*(personalbest_x(i,:)- x(i,:)); % 由个人和全局的最佳信息数据进行更新移动速度
    ...
    x0 = -5:0.01:5;
    y0 = 2.1*(1 - x0 + 2 * x0.^2).*exp(-x0.^2/2);
    plot(x0,y0,'m-','linewidth',3); % m表示的是粉红色,-是表示的是连续的曲线线
    hold on;
    plot(personalbest_x, 1./personalbest_faval-1,'kp','linewidth',4);
    pause(0.1);
    hold off;
    k = k + 1;
end
